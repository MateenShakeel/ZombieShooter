...
//                    Debug.Log("TotalSpawned "+_totalZombiesSpawned);
                }
            }
        }
...
//                GameManager.Instance.GameComplete();
              //  LevelManager.Instance.SetFixedZombieLevelsCash();
        ...
        void SpawnThisWaveZombies(bool shouldPlayStartClip)
        {
            ...
            if (shouldPlayStartClip)
            {
                ...
                if (WavesHUD)
                    StartCoroutine(WaveUIText("Wave " + (_currentWave+1) + "/" + _Waves.Count ));
            }
            
            StartCoroutine(SpawnWithDelay(5f));

        }

        IEnumerator SpawnWithDelay(float f)
        {
            var tempArray = _Waves[_currentWave]._myZombies.ToArray();
        ...
        IEnumerator WaveUIText(string text)
        {
            ...
            float et = 0;
            WavesHUD.gameObject.SetActive(true);
            WavesHUD.transform.GetChild(0).GetComponent<UnityEngine.UI.Text>().text = text;
            if (text == "All waves Survived !")
            {
                Time.timeScale = 0.1f;
                WavesHUD.transform.GetChild(WavesHUD.transform.childCount-1).gameObject.SetActive(false);
                WavesHUD.transform.GetChild(0).gameObject.SetActive(true);
                Text WaveshuDText = WavesHUD.transform.GetChild(0).GetComponent<UnityEngine.UI.Text>();
                WaveshuDText.text = "CLEAR";
                WaveshuDText.fontSize = 60 ;
                yield return new WaitForSecondsRealtime(3f);
                Time.timeScale = 1;
                
            }else if (text == "Wave " + _currentWave + " Survived !")
            {
                WavesHUD.GetComponent<Animator>().enabled = false;
                WavesHUD.transform.GetChild(WavesHUD.transform.childCount-1).gameObject.SetActive(false);
            }else if (text == "Wave 2/2")
            {
                WavesHUD.GetComponent<Animator>().enabled = false; 
                WavesHUD.transform.GetChild(WavesHUD.transform.childCount-1).gameObject.SetActive(true);
                InGameProperties.Instance.currentKill.text =  0.ToString();
                InGameProperties.Instance.bodyShotIndex =  0;

            }
            else
            {
                WavesHUD.transform.GetChild(1).gameObject.SetActive(true);
                WavesHUD.transform.GetChild(0).gameObject.SetActive(true);
            }
            ...
            WavesHUD.alpha = 0f;
            WavesHUD.gameObject.SetActive(false);
            // WavesHUD.GetComponent<Animator>().enabled = false;
        }
    }
