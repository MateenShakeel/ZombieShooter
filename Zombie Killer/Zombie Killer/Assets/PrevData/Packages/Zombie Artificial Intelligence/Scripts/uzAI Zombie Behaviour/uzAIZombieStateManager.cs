...
namespace uzAI
{
    ...
    public class uzAIZombieStateManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            // // if (rand > 1)
            // //     eatingBehaviour.Eatingparticle();
        ...
        private void OnAnimatorMove()
        {
            ...
            // if(Time.timeScale == 1)
            // Locomotion.uzAIAgent.speed = (_animator.deltaPosition / Time.unscaledDeltaTime).magnitude;
            Locomotion.uzAIAgent.speed = zombieSpeed;
        }
        ...
        void FixedUpdate()
        {
            ...
            // if (PlayerPrefs.HasKey("ZombieOnBoarding"))
            IdentifyStates();
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (other.gameObject.tag == "AwarenessTrigger")
            {
                ...
                if (other.gameObject.GetComponentInParent<Transform>().tag == "Player")
                {
                    ...
                    chaseBehaviour.CanChaseThisTarget(other.gameObject.GetComponentInParent<playercontroller>().transform);
                    sightBehaviour.visibleTargets.Add(other.gameObject.GetComponentInParent<playercontroller>().transform);
        ...
        public void Die()
        {
           ...
           // GameplayHandler.Instance.KillHandler();
            ...
            // LevelManager.Instance.CheckZombieKill();
            ...
            _animator.runtimeAnimatorController = InGameProperties.Instance.AnimatorController;
            ...
            Debug.Log("deathId " + ZombieHealthStats.DeathID);
    ...
    public class ZombieChaseTargetBehaviour
    {
        ...
        uzAIZombieHealth _aizombieHealth = new uzAIZombieHealth();
        ...
        public void Chase()
        {
            ...
            _uzAIAgent.isStopped = false;
            _uzAIAgent.speed = _stateManager.zombieSpeed;
            //we only calculate Angle
            //only if we are not Turning
            if (!_animator.GetBool("ZombieIsTurning") && !_stateManager.offmeshBehaviour.isTraversing)
            {
                ...
                else if (hitZombie)
                {
                    ...
                    //set locomotion
                    _animator.SetFloat("Vertical", _stateManager.Locomotion.walkAnimation);
                    Debug.Log("Hit Zombie is trueee");
                }
                // else
                // _animator.SetFloat("Vertical", 1f);
                // Debug.Log("Hit Zombie is " + _aizombieHealth.DisableMotion);
            }
        }
    ...
    public class ZombieAttackBehaviour
    {
        ...
        public static uzAIZombieStateManager localInstance;
        public static uzAIZombieStateManager Instance
        {
            get
            {
                localInstance = GameObject.FindObjectOfType<uzAIZombieStateManager>();
        ...
        void SearchAttackTargets()
        {
            ...
            for (int i = 0; i < targetsInViewRadius.Length; i++)
            {
                ...
                if (targetsInViewRadius[i].tag == "Player" || targetsInViewRadius[i].tag == "Barricades")
                {
                    //get the target
                    Transform target = targetsInViewRadius[i].transform;
        ...
        public void Attack()
        {
            ...
            // _stateManager.currentZombieState = ZombieStates.Attacking;
            ////if(true)
            if (Vector3.Distance(_stateManager.playerpos.position, _stateManager.transform.position) <= 5)
            {
                // canAttackPlayer = true;
                //if(canAttackPlayer)
                //{
                ...
                //go to Idle
                _stateManager.Locomotion.walkAnimation =
                    Mathf.Lerp(_stateManager.Locomotion.walkAnimation, 0, Time.deltaTime * 5);
                _animator.SetFloat("Vertical", _stateManager.Locomotion.walkAnimation);
                ...
                if (!isAttacking)
                {
                    ...
                    if (_timer >= AttackDelay)
                    {
                        ...
                        int Attack_Anim = Random.Range(0, 2);
                        Debug.Log("5");
                ...
                if (canAttack)
                {
                    ...
                    if (!hasAttacked)
                    {
                        ...
                        if (Player && Barricade == null && ShooterAI == null)
                        {
                            ...
                            {
                                Debug.Log("111111");
                                Player.Damage(Random.Range(minDamage, maxDamage));
                                // cameraShake.Instance.ShakeStart();
                                //Indicator 
                                // InGameProperties.Instance.IndicatorArrow(_animator.gameObject.transform);
                        ...
                        if (_stateManager.ZombieSFX._attackSFX.Count > 0)
                            _stateManager.ZombieSFX.PlaySFX(
                                _stateManager.ZombieSFX._attackSFX[
                                    Random.Range(0, _stateManager.ZombieSFX._attackSFX.Count)]);
    ...
    public class uzAIZombieHealth
    {
        ...
        int i = 1;
        private float hitReactionDelay = 2f;
        private float lastplay;

        void HitReaction()
        {
            if (Time.time >= lastplay + hitReactionDelay)
            {
        ...
        public void ZombieGotHit()
        {
            ...
            if (DisableMotion)
            {
                if ((_uzAIAgent.remainingDistance - _uzAIAgent.stoppingDistance ) > 0 && _stateManager.currentZombieState == ZombieStates.Chasing && !_stateManager.attackBehaviour.isattackStart)
                {
                    _stateManager.zombieSpeed = 2f;
                    _uzAIAgent.speed = _stateManager.zombieSpeed;
                    //lerp speed to 0
                    ...
                    _uzAIAgent.isStopped = false;
                    _animator.SetFloat("Vertical", 2);
                    // Debug.Log("Distance Pending " +  _stateManager.Locomotion.walkAnimation);
                }else if (CurrentHealth > 0 && _stateManager.currentZombieState == ZombieStates.Attacking)
                {
                    _stateManager.attackBehaviour.Attack();
                }
            }else
                _animator.SetFloat("Vertical", 1f);
    ...
    public class uzAIZombieSFX
    {
        ...
        public void Initialize(Transform _t)
        {
            ...
            //Add Audio Source
            _source =  _uzAI.gameObject.GetComponent<AudioSource>();
        }
