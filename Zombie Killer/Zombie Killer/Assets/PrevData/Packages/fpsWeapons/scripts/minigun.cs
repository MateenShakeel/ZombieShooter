...
public class minigun : MonoBehaviour {
	...
	void Update () 
	{
		...
		if (Physics.Raycast(raycastPoint.position, raycastPoint.TransformDirection(Vector3.forward), out hit,
					Mathf.Infinity))
		{
		...
		else
		{
			...
			if ((ControlFreak2.CF2Input.GetButton("Fire2")|| ControlFreak2.CF2Input.GetButton("Fire1") || ControlFreak2.CF2Input.GetAxis("Aim") > 0.1) && canaim && !playercontrol.running)
		...
		if (Time.timeScale > 0)
		{
			servoAudioSource.gameObject.SetActive(true);
		}
		else
		{
			servoAudioSource.gameObject.SetActive(false);
		}
	}
