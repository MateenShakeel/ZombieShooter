	...
	void Update () 
	{
		...
		{	
			...
			if (ammo <= 0f)
			{
				canfire = false;
				canreload = false;
				if ((ControlFreak2.CF2Input.GetButton("Fire2") && canfire || ControlFreak2.CF2Input.GetAxis ("Fire1")>0.1) )
				{
		...
		if ((ControlFreak2.CF2Input.GetButton("Fire2") || ControlFreak2.CF2Input.GetAxis ("Fire1")>0.1) && canfire && !isreloading && !myanimation.IsPlaying (readyAnim.name)&& !myanimation.IsPlaying (hideAnim.name))
		{
