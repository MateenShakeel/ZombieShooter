...
public class RPG : MonoBehaviour {
	...
	public AnimationClip saluteAnim;
	...
	void Update () 
	{
		...
		if (Physics.Raycast(muzzle.position, transform.TransformDirection(Vector3.forward), out hit, Mathf.Infinity))
		{                                                                   
			...
			else
			{
				...
				{
					enemy.enabled = false;
					if (PlayerPrefs.GetString("Mode") != "TeamDeath" && PlayerPrefs.GetString("Mode") != "Survival")
					{
	...
	void SaluteAnim()
	{
		myanimation.Play(saluteAnim.name);
	}
