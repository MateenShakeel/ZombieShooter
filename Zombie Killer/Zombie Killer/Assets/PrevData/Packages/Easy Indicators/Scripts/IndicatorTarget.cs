...
public class IndicatorTarget : MonoBehaviour
{
	...
	//  Check if the viewer is on-screen or off-screen
	private bool isOnScreenEnabled;
	//  Check if onscreen is enabled
	private bool isOffScreenEnabled;
	//  Check if offscreen is enabled.
	...
	public void InitializeIndicator ()
	{
		...
		try {
			...
			if (IPanel.OnScreenImage != null) {

				IPanel.OnScreenImage.sprite = OnScreenSprite;
				IPanel.OnScreenImage.preserveAspect = true;



				if (transform.GetComponent <BaseScript> () == null) {

					IPanel.OnScreenImage.transform.localScale = new Vector3 (1f, 1f, 1f);
					IPanel.OffScreenImage.transform.localScale = new Vector3 (1f, 1f, 1f);
