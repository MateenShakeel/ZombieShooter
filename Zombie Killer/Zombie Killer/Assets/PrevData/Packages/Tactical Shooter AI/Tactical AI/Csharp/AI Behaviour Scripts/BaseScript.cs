...
namespace TacticalAI
{
    public class BaseScript : MonoBehaviour
    {
        ...
        [HideInInspector] public CoverNodeScript currentCoverNodeScript;
        ...
        [HideInInspector] public Vector3 dodgeClearHeightCheckPos;
        [HideInInspector] public float origAcceleration;
...
//            StartCoroutine(WaitBetweenDodges());
            ...
            {
                StartCoroutine(nameof(AICycle));
            }
            else
            {
                StartCoroutine(nameof(PerformanceAiCycle));
            }
        }
        ...
        IEnumerator AICycle()
        {
            ...
            while (this.enabled)
            {
                ...
                if (!isDodging)
                {
                    if ((myTransform && targetTransform && canMelee && !isMeleeing &&
                         Vector3.SqrMagnitude(myTransform.position - targetTransform.position) < meleeRange))
                    {
                        isSprinting = false;
                    ...
                    {
                        if (!isMeleeing &&
                            Vector3.SqrMagnitude(myTransform.position - currentBehaviour.targetVector) >
                            distFromTargetToSprint && canSprint && engaging)
                        {
                ...
                else if (Vector3.SqrMagnitude(myTransform.position - currentBehaviour.targetVector) <
                         distFromTargetToSprint && engaging && !isDodging)
                {
                    animationScript.StopSprinting();
                    SetProperSpeed();
        ...
        private bool usingOverrideBehaviour = false;
        ...
        public CustomAIBehaviour GetNewBehaviour(AIBehaviour t)
        {
            ...
            switch (t)
            {
                case AIBehaviour.Search:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.Search));
                    break;
                case AIBehaviour.Dodging:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.Dodge));
                    break;
                case AIBehaviour.Cover:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.Cover));
                    break;
                case AIBehaviour.Patrolling:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.Patrol));
                    break;
                case AIBehaviour.Wander:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.Wander));
                    break;
                case AIBehaviour.GoToMoveTarget:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.ChargeTarget));
                    break;
                case AIBehaviour.UseDynamicObject:
                    bToReturn =
                        (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.DynamicObject));
                    break;
                case AIBehaviour.InvestigateSound:
                    bToReturn =
                        (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.InvestigateSound));
                    break;
                case AIBehaviour.MoveToTransform:
                    bToReturn =
                        (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.MoveToTransform));
                    break;
                case AIBehaviour.RunFromGrenade:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(
                        typeof(TacticalAI.RunAwayFromGrenade));
                    break;
                case AIBehaviour.Skirmish:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.Skirmish));
                    break;
                case AIBehaviour.Stagger:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.Stagger));
                    break;
                case AIBehaviour.Parkour:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.Parkour));
                    break;
                case AIBehaviour.ThrowGrenade:
                    bToReturn = (TacticalAI.CustomAIBehaviour) gameObject.AddComponent(typeof(TacticalAI.ThrowGrenade));
                    break;
            }
        ...
        private void LateUpdate()
        {
            ...
            if (!inCover && !ControllerScript.pMode && !inParkour && !isDodging && !isMeleeing && !isStaggered &&
                !navI.OnNavmeshLink())
            {
        ...
        public void CheckToSeeIfWeShouldDodge()
        {
            ...
            if (shouldTryAndDodge && myTransform && targetTransform && !inCover && !isDodging && !isSprinting &&
                !isStaggered && !Physics.Linecast(myTransform.position, targetTransform.position, layerMask) &&
                Vector3.Distance(myTransform.position, targetTransform.position) > minDistToDodge &&
                !gunScript.IsFiring())
            {
                if (canOverrideBehaviour)
                {
                    SetOverrideBehaviour(GetNewBehaviour(AIBehaviour.Dodging), false);
                    StartCoroutine(WaitBetweenDodges());
                }
            }
        }
        ...
        [HideInInspector] public bool usingDynamicObject = false;
        public bool canUseDynamicObject = true;

        public bool SetDynamicObject(Transform newMovementObjectTransform, string anim, string newMethodToCall,
            bool requireEngaging)
        {
            return SetDynamicObject(newMovementObjectTransform, anim, newMethodToCall, requireEngaging, 1.0f);
        }

        public bool SetDynamicObject(Transform newMovementObjectTransform, string anim, string newMethodToCall,
            bool requireEngaging, float timeToWait)
        {
        ...
        {
            if (!isDodging && !isMeleeing && !inParkour && !isStaggered && !navI.OnNavmeshLink() &&
                (!useAdvancedCover || !inCover))
            {
                isStaggered = true;
                SetOverrideBehaviour(GetNewBehaviour(AIBehaviour.Stagger), true);
                WaitBetweenDodges();
            }
        }
