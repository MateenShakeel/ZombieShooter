    ...
    public class AnimationScript : MonoBehaviour
    {
        ...
        void AnimateAI()
        {
            ...
            if (!onLink && !myBaseScript.isStaggered)
            {
                ...
                float f = Vector3.Dot(myAIBodyTransform.forward, navi.GetDesiredVelocity()) / maxMovementSpeed;
                float f1 = Vector3.Dot(myAIBodyTransform.right, navi.GetDesiredVelocity()) / maxMovementSpeed;
                if (float.IsNaN(f))
                {
                    f = 0;
                }
                ...
                animator.SetFloat(forwardsMoveHash, f, dampTimeNow, Time.deltaTime);
                animator.SetFloat(sidewaysMoveHash, f1, dampTimeNow, Time.deltaTime);
            }
                ...
                else if ((myBaseScript.IsEnaging() && myBaseScript.targetTransform && !sprinting))
                {
                    ...
                    else
                    {
                        ...
                        if (angleBetweenFor < 90)
                        {
                            ...
                            float f = Vector3.Magnitude(navi.GetDesiredVelocity()) / maxMovementSpeed;
                            if (float.IsNaN(f))
                            {
                                f = 0;
                            }
                            animator.SetFloat(forwardsMoveHash, f, animationDampTime, Time.deltaTime);
                            animator.SetFloat(sidewaysMoveHash, 0, animationDampTime, Time.deltaTime);
                        }
                        else
                        {
